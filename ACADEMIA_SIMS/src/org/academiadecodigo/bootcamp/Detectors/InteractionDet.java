...
public class InteractionDet {
    ...
    public InteractionDet(int xMin, int xMax, int yMin, int yMax, String text, Actions action) {
        this.xMin = xMin;
        this.yMin = yMin;
        this.xMax = xMax;
        this.yMax = yMax;
    ...
    public boolean interactionWith(int x, int y) {
        ...
        if ((x >= this.xMin && x <= this.xMax) && (y >= yMin && y <= yMax)) {
            Text text = new Text(85, 40, this.text);
            text.draw();
            return true;
        } else {
            Rectangle newRectangle = new Rectangle(10, 40, 240, 20);
    ...
    public int[] setNecessities(int[] necessities) throws InterruptedException {
